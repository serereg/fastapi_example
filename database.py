...
engine = create_engine(
    "mysql+mysqlconnector://root:my-secret-pw@localhost:3306/books", echo=True
)
...
class BookAuthor(Base):
    ...
    def __repr__(self):
        return "<BookAuthor (bookauthor_id='{0}', author_id='{1}', book_id='{2}', first_name'{3}')>".format(
            self.bookauthor_id, self.author_id, self.book_id, self.author.first_name
        )
...
def add_book(book: Book, author: Author):
    with Session(engine) as session:
        ...
        if existing_author is not None:
            print("Author exist! Adding book")
            session.flush()
            pairing = BookAuthor(
                author_id=existing_author.author_id, book_id=book.book_id
            )
        else:
            print("Author doesn't exist! Adding author")
            session.add(author)
            session.flush()
...
def get_book(book_id: int):
    with Session(engine) as session:
        ...
        if not book:
            raise Exception("The book does not exist")
        pairing = session.execute(
            select(BookAuthor).filter(BookAuthor.book_id == book_id)
        ).scalar()
        author = session.execute(
            select(Author).filter(Author.author_id == pairing.author_id)
        ).scalar()
        return book, author
